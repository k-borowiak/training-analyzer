...
def _refresh_tokens(refresh_token: str) -> dict:
    """
    Wymienia refresh_token na nowe access_token + refresh_token + expires_at
    """
...
def get_valid_access_token(db: DbSession, user_id: int) -> str:
    ...
    Jeśli wygasł, robi refresh i aktualizuje DB.
    """
    ...
    now = int(time.time())
    # zapas 60 sekund, żeby nie trafić w moment wygaśnięcia
...
def strava_get(db: DbSession, user_id: int, path: str, params: dict | None = None) -> dict:
    """
    Prosty GET do Strava API z automatycznym refresh tokenów.
    """
